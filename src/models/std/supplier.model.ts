...
@Table({
  tableName: 'STD_SUPPLIER_TB',
  modelName: 'StdSupplier',
  comment: '공급처 정보 테이블',
  timestamps: true,
  underscored: true,
})
export default class StdSupplier extends Model<IStdSupplier> {
  @Column({
    comment: '공급처ID',
    primaryKey: true,
    autoIncrement: true,
  ...
  @Unique('std_supplier_tb_partner_id_supplier_cd_un')
  ...
  @Unique('std_supplier_tb_uuid_un')
  @Column({
    comment: '공급처UUID',
    type: DataType.UUID,
    allowNull: false,
