...
import OutReceive from './receive.model';

@Table({
  tableName: 'OUT_RECEIVE_DETAIL_TB',
  modelName: 'OutReceiveDetail',
  comment: '외주입하 상세정보 테이블',
  timestamps: true,
  underscored: true,
})
export default class OutReceiveDetail extends Model<IOutReceiveDetail> {
  @Column({
    comment: '외주입하 상세ID',
    primaryKey: true,
    autoIncrement: true,
  ...
  @Unique('out_receive_detail_tb_receive_id_seq_un')
  @ForeignKey(() => OutReceive)
  @Column({
    comment: '외주입하ID',
    type: DataType.INTEGER,
    allowNull: false,
  ...
  @Unique('out_receive_detail_tb_uuid_un')
  @Column({
    comment: '외주입하 상세UUID',
    type: DataType.UUID,
    allowNull: false,
  ...
  @BelongsTo(() => OutReceive, { foreignKey: 'receive_id', targetKey: 'receive_id', onDelete: 'restrict', onUpdate: 'cascade' })
  outReceive: OutReceive;
