...
class AdmCompanyOptRepo {
  ...
  //#region ✅ CRUD Functions
	...
	public create = async(body: IAdmCompanyOpt[], uid: number, transaction?: Transaction) => {
		try {
			const promises = body.map((companyOpt: any) => {
    ...
    try {
      ...
      const promises = body.map((companyOpt: any) => {
        return this.repo.update(
          {
            company_opt_cd: companyOpt.company_opt_cd ?? null,
						company_opt_nm: companyOpt.company_opt_nm ?? null,
						remark: companyOpt.remark ?? null,
						val: companyOpt.val ?? null,
						val_opt: companyOpt.val_opt ?? null,
						sortby: companyOpt.sortby ?? null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IAdmCompanyOpt[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((companyOpt: any) => {
        return this.repo.destroy({ where: { uuid: companyOpt.uuid }, transaction});
      });
