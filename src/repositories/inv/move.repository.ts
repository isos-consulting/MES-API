...
import InvMove from '../../models/inv/move.model';
import IInvMove from '../../interfaces/inv/move.interface';

class InvMoveRepo {
  repo: Repository<InvMove>;
  sequelize: Sequelize;
  tenant: string;
  ...
  constructor(tenant: string) {
    ...
    this.repo = this.sequelize.getRepository(InvMove);
  }
  //#endregion
  ...
  public create = async(body: IInvMove[], uid: number, transaction?: Transaction) => {
    try {
      const move = body.map((move) => {
        return {
          factory_id: move.factory_id,
          reg_date: move.reg_date,
          prod_id: move.prod_id,
          lot_no: move.lot_no,
          qty: move.qty,
          from_store_id: move.from_store_id,
          from_location_id: move.from_location_id,
          to_store_id: move.to_store_id,
      ...
      const result = await this.repo.bulkCreate(move, { individualHooks: true, transaction });
  ...
  public read = async(params?: any) => {
    try {
      const result = await this.repo.findAll({ 
        ...
        attributes: [
          [ Sequelize.col('invMove.uuid'), 'move_uuid' ],
        ...
        where: { 
          [Op.and]: [
            Sequelize.where(Sequelize.fn('date', Sequelize.col('invMove.reg_date')), '>=', params.start_date),
            Sequelize.where(Sequelize.fn('date', Sequelize.col('invMove.reg_date')), '<=', params.end_date),
          ]
        },
  ...
  public update = async(body: IInvMove[], uid: number, transaction?: Transaction) => {
    let raws: any[] = [];

    try {
      const previousRaws = await getPreviousRaws(body, this.repo);

      for await (let move of body) {
        const result = await this.repo.update(
          {
            qty: move.qty != null ? move.qty : null,
            remark: move.remark != null ? move.remark : null,
            updated_uid: uid,
          } as any,
          { 
            where: { uuid: move.uuid },
            returning: true,
      ...
      await new AdmLogRepo(this.tenant).create('update', this.sequelize.models.InvMove.getTableName() as string, previousRaws, uid, transaction);
  ...
  public patch = async(body: IInvMove[], uid: number, transaction?: Transaction) => {
    let raws: any[] = [];
  ...
  public delete = async(body: IInvMove[], uid: number, transaction?: Transaction) => {
    let count: number = 0;
    ...
    try {      
      ...
      for await (let move of body) {
        count += await this.repo.destroy({ where: { uuid: move.uuid }, transaction});
      };

      await new AdmLogRepo(this.tenant).create('delete', this.sequelize.models.InvMove.getTableName() as string, previousRaws, uid, transaction);
