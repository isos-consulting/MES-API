...
class InvStockRejectRepo {
  ...
  public create = async(body: IInvStockReject[], uid: number, transaction?: Transaction) => {
    try {
      const promises = body.map((stockReject: any) => {
    ...
    try {
      ...
      const promises = body.map((stockReject: any) => {
        return this.repo.update(
          {
            qty: stockReject.qty ?? null,
            remark: stockReject.remark ?? null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IInvStockReject[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((stockReject: any) => {
        return this.repo.destroy({ where: { uuid: stockReject.uuid }, transaction});
      });
