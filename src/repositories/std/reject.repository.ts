...
class StdRejectRepo {
  ...
  //#region ✅ Constructor
  constructor() {
    this.repo = sequelize.getRepository(StdReject);
  }
  //#endregion
      ...
      const result = await this.repo.findAll({ 
        include: [
          ...
          { 
            ...
            attributes: [],
            required: false,
            where: { uuid: params.reject_type_uuid ? params.reject_type_uuid : { [Op.ne]: null } }
          },
        ...
        attributes: [
          ...
          [ Sequelize.col('stdRejectType.reject_type_cd'), 'reject_type_cd' ],
          [ Sequelize.col('stdRejectType.reject_type_nm'), 'reject_type_nm' ],
          'created_at',
          [ Sequelize.col('createUser.user_nm'), 'created_nm' ],
          'updated_at',
          [ Sequelize.col('updateUser.user_nm'), 'updated_nm' ]
        ],
        order: [ 'factory_id', 'reject_type_id', 'reject_id' ],
      });
    ...
    try {
      const result = await this.repo.findOne({ 
        include: [
          ...
          { model: sequelize.models.StdRejectType, attributes: [], required: false },
          { model: sequelize.models.AutUser, as: 'createUser', attributes: [], required: true },
          { model: sequelize.models.AutUser, as: 'updateUser', attributes: [], required: true },
        ],
        attributes: [
  ...
  public readRawByUnique = async(
    params: { factory_id: number, reject_cd: string }
  ) => {
    const result = await this.repo.findOne({ 
      where: {
        [Op.and]: [
          { factory_id: params.factory_id },
          { reject_cd: params.reject_cd }
        ]
      }
    });
    return convertReadResult(result);
  ...
  public update = async(body: IStdReject[], uid: number, transaction?: Transaction) => {
    ...
    try {
      ...
      await new AdmLogRepo().create('update', sequelize.models.StdReject.getTableName() as string, previousRaws, uid, transaction);
  ...
  public delete = async(body: IStdReject[], uid: number, transaction?: Transaction) => {
    ...
    try {      
      ...
      await new AdmLogRepo().create('delete', sequelize.models.StdReject.getTableName() as string, previousRaws, uid, transaction);
