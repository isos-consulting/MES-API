...
class StdUnitRepo {
  ...
  //#region ✅ Constructor
  constructor() {
    this.repo = sequelize.getRepository(StdUnit);
  }
  //#endregion
  ...
  public create = async(body: IStdUnit[], uid: number, transaction?: Transaction) => {
    try {
      ...
      const result = await this.repo.bulkCreate(unit, { individualHooks: true, transaction });
  ...
  public update = async(body: IStdUnit[], uid: number, transaction?: Transaction) => {
    ...
    try {
      ...
      for await (let unit of body) {
        const result = await this.repo.update(
          {
            unit_cd: unit.unit_cd != null ? unit.unit_cd : null,
            unit_nm: unit.unit_nm != null ? unit.unit_nm : null,
            updated_uid: uid,
          } as any,
          { 
      ...
      await new AdmLogRepo().create('update', sequelize.models.StdUnit.getTableName() as string, previousRaws, uid, transaction);
  ...
  public delete = async(body: IStdUnit[], uid: number, transaction?: Transaction) => {
    ...
    try {      
      ...
      for await (let unit of body) {
        count += await this.repo.destroy({ where: { uuid: unit.uuid }, transaction});
      };

      await new AdmLogRepo().create('delete', sequelize.models.StdUnit.getTableName() as string, previousRaws, uid, transaction);
