...
class StdGradeRepo {
  ...
  //#region ✅ Constructor
  constructor() {
    this.repo = sequelize.getRepository(StdGrade);
  }
  //#endregion
  ...
  public create = async(body: IStdGrade[], uid: number, transaction?: Transaction) => {
    try {
      ...
      const result = await this.repo.bulkCreate(grade, { individualHooks: true, transaction });
  ...
  public update = async(body: IStdGrade[], uid: number, transaction?: Transaction) => {
    ...
    try {
      ...
      for await (let grade of body) {
        const result = await this.repo.update(
          {
            grade_cd: grade.grade_cd != null ? grade.grade_cd : null,
            grade_nm: grade.grade_nm != null ? grade.grade_nm : null,
            updated_uid: uid,
          } as any,
          { 
      ...
      await new AdmLogRepo().create('update', sequelize.models.StdGrade.getTableName() as string, previousRaws, uid, transaction);
  ...
  public delete = async(body: IStdGrade[], uid: number, transaction?: Transaction) => {
    ...
    try {      
      ...
      for await (let grade of body) {
        count += await this.repo.destroy({ where: { uuid: grade.uuid }, transaction});
      };

      await new AdmLogRepo().create('delete', sequelize.models.StdGrade.getTableName() as string, previousRaws, uid, transaction);
