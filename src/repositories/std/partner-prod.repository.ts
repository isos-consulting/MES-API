...
class StdPartnerProdRepo {
  ...
  public create = async(body: IStdPartnerProd[], uid: number, transaction?: Transaction) => {
    try {
      const partnerProd = body.map((partnerProd) => {
        return {
      ...
      const result = await this.repo.bulkCreate(partnerProd, { individualHooks: true, transaction });
  ...
  public update = async(body: IStdPartnerProd[], uid: number, transaction?: Transaction) => {
    ...
    try {
      ...
      for await (let partnerProd of body) {
        const result = await this.repo.update(
          {
            partner_prod_no: partnerProd.partner_prod_no != null ? partnerProd.partner_prod_no : null,
            remark: partnerProd.remark != null ? partnerProd.remark : null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IStdPartnerProd[], uid: number, transaction?: Transaction) => {
    ...
    try {      
      ...
      for await (let partnerProd of body) {
        count += await this.repo.destroy({ where: { uuid: partnerProd.uuid }, transaction});
      };
