...
class StdVendorPriceRepo {
  ...
  public create = async(body: IStdVendorPrice[], uid: number, transaction?: Transaction) => {
    try {
      const promises = body.map((vendorPrice: any) => {
    ...
    try {
      ...
      const promises = body.map((vendorPrice: any) => {
        return this.repo.update(
          {
            unit_id: vendorPrice.unit_id ?? null,
            price: vendorPrice.price ?? null,
            money_unit_id: vendorPrice.money_unit_id ?? null,
            price_type_id: vendorPrice.price_type_id ?? null,
            start_date: vendorPrice.start_date ?? null,
            retroactive_price: vendorPrice.retroactive_price ?? null,
            division: vendorPrice.division ?? null,
            remark: vendorPrice.remark ?? null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IStdVendorPrice[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((vendorPrice: any) => {
        return this.repo.destroy({ where: { uuid: vendorPrice.uuid }, transaction});
      });
