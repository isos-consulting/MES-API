...
class StdMoneyUnitRepo {
  ...
  constructor(tenant: string) {
    ...
    this.repo = this.sequelize.getRepository(StdMoneyUnit);
  }
  //#endregion
  ...
  public create = async(body: IStdMoneyUnit[], uid: number, transaction?: Transaction) => {
    try {
      const promises = body.map((moneyUnit: any) => {
    ...
    try {
      ...
      const promises = body.map((moneyUnit: any) => {
        return this.repo.update(
          {
            money_unit_cd: moneyUnit.money_unit_cd ?? null,
            money_unit_nm: moneyUnit.money_unit_nm ?? null,
            updated_uid: uid,
          } as any,
          { 
      ...
      await new AdmLogRepo(this.tenant).create('update', this.sequelize.models.StdMoneyUnit.getTableName() as string, previousRaws, uid, transaction);
  ...
  public delete = async(body: IStdMoneyUnit[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((moneyUnit: any) => {
        return this.repo.destroy({ where: { uuid: moneyUnit.uuid }, transaction});
      });
      const count = _.sum(await Promise.all(promises));

      await new AdmLogRepo(this.tenant).create('delete', this.sequelize.models.StdMoneyUnit.getTableName() as string, previousRaws, uid, transaction);
