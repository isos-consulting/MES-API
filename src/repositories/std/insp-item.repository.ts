...
class StdInspItemRepo {
  ...
  //#region ✅ Constructor
  constructor() {
    this.repo = sequelize.getRepository(StdInspItem);
  }
  //#endregion
    ...
    try {
      const result = await this.repo.findAll({ 
        include: [
          ...
          { 
            model: sequelize.models.StdInspItemType, 
            attributes: [],
  ...
  public update = async(body: IStdInspItem[], uid: number, transaction?: Transaction) => {
    ...
    try {
      ...
      await new AdmLogRepo().create('update', sequelize.models.StdInspItem.getTableName() as string, previousRaws, uid, transaction);
  ...
  public delete = async(body: IStdInspItem[], uid: number, transaction?: Transaction) => {
    ...
    try {      
      ...
      await new AdmLogRepo().create('delete', sequelize.models.StdInspItem.getTableName() as string, previousRaws, uid, transaction);
