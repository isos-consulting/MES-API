...
class StdItemTypeRepo {
  ...
  public create = async(body: IStdItemType[], uid: number, transaction?: Transaction) => {
    try {
      const promises = body.map((itemType: any) => {
    ...
    try {
      ...
      const promises = body.map((itemType: any) => {
        return this.repo.update(
          {
            item_type_cd: itemType.item_type_cd ?? null,
            item_type_nm: itemType.item_type_nm ?? null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IStdItemType[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((itemType: any) => {
        return this.repo.destroy({ where: { uuid: itemType.uuid }, transaction});
      });
