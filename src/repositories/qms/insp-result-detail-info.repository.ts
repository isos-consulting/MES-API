...
class QmsInspResultDetailInfoRepo {
  ...
  public create = async(body: IQmsInspResultDetailInfo[], uid: number, transaction?: Transaction) => {
    try {
      const promises = body.map((inspDetailInfo: any) => {
    ...
    try {
      ...
      const promises = body.map((inspDetailInfo: any) => {
        return this.repo.update(
          {
            insp_result_fg: inspDetailInfo.insp_result_fg ?? null,
            remark: inspDetailInfo.remark ?? null,
            updated_uid: uid,
          } as any,
          { 
  ...
  public delete = async(body: IQmsInspResultDetailInfo[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const promises = body.map((inspDetailInfo: any) => {
        return this.repo.destroy({ where: { uuid: inspDetailInfo.uuid }, transaction});
      });
  ...
  public deleteByResultIds = async(ids: number[], uid: number, transaction?: Transaction) => {
    try {      
      ...
      const count = await this.repo.destroy({ where: { insp_result_id: ids }, transaction});
