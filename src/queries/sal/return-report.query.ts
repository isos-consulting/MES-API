  ...
  switch (params.sub_total_type) {
    case 'partner':
      insertTotalToTempTable = `
        INSERT INTO temp_return(sort, partner_id, qty, supply_price, tax, total_price, outgo_qty)
        SELECT 
          CASE WHEN t_r.partner_id IS NOT NULL THEN 2 ELSE 3 END,
          t_r.partner_id, sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.outgo_qty)
    ...
    case 'prod':
      insertTotalToTempTable = `
        INSERT INTO temp_return(sort, prod_id, qty, supply_price, tax, total_price, outgo_qty)
        SELECT 
          CASE WHEN t_r.prod_id IS NOT NULL THEN 2 ELSE 3 END,
          t_r.prod_id, sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.outgo_qty)
        FROM temp_return t_r
    ...
    case 'date':
      insertTotalToTempTable = `
        INSERT INTO temp_return(sort, reg_date, qty, supply_price, tax, total_price, outgo_qty)
        SELECT 
          CASE WHEN t_r.reg_date IS NOT NULL THEN 2 ELSE 3 END,
          t_r.reg_date, sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.outgo_qty)
        FROM temp_return t_r
    ...
    case 'none':
      insertTotalToTempTable = `
        INSERT INTO temp_return(sort, qty, supply_price, tax, total_price, outgo_qty)
        SELECT 3, sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.outgo_qty)
        FROM temp_return t_r;
      `;
