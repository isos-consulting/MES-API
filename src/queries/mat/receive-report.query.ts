  ...
  switch (params.sub_total_type) {
    case 'partner':
      insertTotalToTempTable = `
        INSERT INTO temp_receive(sort, partner_id, order_qty, qty, supply_price, tax, total_price, income_qty, reject_qty)
        SELECT 
          CASE WHEN t_r.partner_id IS NOT NULL THEN 2 ELSE 3 END,
          t_r.partner_id, sum(t_r.order_qty), sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.income_qty), sum(t_r.reject_qty)
    ...
    case 'prod':
      insertTotalToTempTable = `
        INSERT INTO temp_receive(sort, prod_id, order_qty, qty, supply_price, tax, total_price, income_qty, reject_qty)
        SELECT 
          CASE WHEN t_r.prod_id IS NOT NULL THEN 2 ELSE 3 END,
          t_r.prod_id, sum(t_r.order_qty), sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.income_qty), sum(t_r.reject_qty)
        FROM temp_receive t_r
    ...
    case 'date':
      insertTotalToTempTable = `
        INSERT INTO temp_receive(sort, reg_date, order_qty, qty, supply_price, tax, total_price, income_qty, reject_qty)
        SELECT 
          CASE WHEN t_r.reg_date IS NOT NULL THEN 2 ELSE 3 END,
          t_r.reg_date, sum(t_r.order_qty), sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.income_qty), sum(t_r.reject_qty)
        FROM temp_receive t_r
    ...
    case 'none':
      insertTotalToTempTable = `
        INSERT INTO temp_receive(sort, order_qty, qty, supply_price, tax, total_price, income_qty, reject_qty)
        SELECT 3, sum(t_r.order_qty), sum(t_r.qty), sum(t_r.supply_price), sum(t_r.tax), sum(t_r.total_price), sum(t_r.income_qty), sum(t_r.reject_qty)
        FROM temp_receive t_r;
      `;

      reportOrderBy = `ORDER BY t_r.receive_detail_id, t_r.sort;`;
      break;
    default: 
      insertTotalToTempTable = '';
      reportOrderBy = '';
