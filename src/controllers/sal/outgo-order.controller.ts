...
import SalOutgoService from '../../services/sal/outgo-order.service';
import SalOutgoDetailService from '../../services/sal/outgo-order-detail.service';
...
import SalOutgoOrderService from '../../services/sal/outgo-order.service';
import SalOutgoOrderDetailService from '../../services/sal/outgo-order-detail.service';

class SalOutgoOrderCtl {
  stateTag: string

  //#region ✅ Constructor
  constructor() {
    this.stateTag = 'salOutgoOrder'
  };
  //#endregion
  ...
  public create = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const data = {
        ...
        let outgoOrderUuid: string;
        let outgoOrderId: number;
        let maxSeq: number;
        let headerResult: ApiResult<any> = { count: 0, raws: [] };

        // 📌 자재발주의 UUID가 입력되지 않은 경우 자재발주 신규 발행
        if (!data.header.uuid) {
          // 📌 전표자동발행 옵션 여부 확인
          const hasAutoOption = await patternOptService.hasAutoOption({ table_nm: 'SAL_OUTGO_ORDER_TB', col_nm: 'stmt_no', tran });
          ...
          outgoOrderUuid = headerResult.raws[0].uuid;
          outgoOrderId = headerResult.raws[0].outgo_order_id;
          maxSeq = 0;
        } else {
          outgoOrderUuid = data.header.uuid;
          outgoOrderId = data.header.outgo_order_id;

          // 📌 Max Seq 계산
          maxSeq = await detailService.getMaxSeq(outgoOrderId, tran) as number;
        }

        // 📌 생성된 발주ID 입력 및 Max Seq 기준 Seq 발행
        ...
        headerResult = await service.updateTotal(outgoOrderId, outgoOrderUuid, req.user?.uid as number, tran);
  ...
  // 📒 Fn[readIncludeDetails]: 자재발주 데이터의 Header + Detail 함께 조회
  public readIncludeDetails = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const detailsResult = await detailService.read({ ...params, outgo_order_uuid: params.uuid });
  ...
  // 📒 Fn[readDetails]: 자재발주 데이터의 Detail 조회
  public readDetails = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      const params = matchedData(req, { locations: [ 'query', 'params' ] });
      const detailService = new SalOutgoDetailService(req.tenant.uuid);
      
      const result = await detailService.read({ ...params, outgo_order_uuid: params.uuid });
  ...
  public update = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      await sequelizes[req.tenant.uuid].transaction(async(tran: any) => { 
        ...
        // 📌 자재발주의 합계수량 및 합계금액 계산
        const outgoOrderId = headerResult.raws[0].outgo_order_id;
        const outgoOrderUuid = headerResult.raws[0].uuid;
        headerResult = await service.updateTotal(outgoOrderId, outgoOrderUuid, req.user?.uid as number, tran);
  ...
  public patch = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      let result: ApiResult<any> = { count:0, raws: [] };
      const service = new SalOutgoOrderService(req.tenant.uuid);
      const detailService = new SalOutgoOrderDetailService(req.tenant.uuid);
      const matched = matchedData(req, { locations: [ 'body' ] });
    ...
    try {
      ...
      await sequelizes[req.tenant.uuid].transaction(async(tran: any) => { 
        ...
        if (count == 0) {
          ...
          headerResult = await service.updateTotal(data.header.outgo_order_id, data.header.uuid, req.user?.uid as number, tran);
        }
