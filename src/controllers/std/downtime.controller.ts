import StdDowntimeTypeRepo from '../../repositories/std/downtime-type.repository';
...
class StdDowntimeCtl extends BaseCtl {
  ...
  repo: StdDowntimeRepo;

  //#region ✅ Constructor
  constructor() {
    // ✅ 부모 Controller (Base Controller) 의 CRUD Function 과 상속 받는 자식 Controller(this) 의 Repository 를 연결하기 위하여 생성자에서 Repository 생성
    super(new StdDowntimeRepo());
    ...
    this.fkIdInfos = [
      ...
      {
        key: 'downtimeType',
        repo: new StdDowntimeTypeRepo(),
        idName: 'downtime_type_id',
        uuidName: 'downtime_type_uuid'
      },
    ];
  };
  //#endregion
  ...
  public convertUniqueToFk = async (body: any[]) => {
    ...
    const downtimeTypeRepo = new StdDowntimeTypeRepo();
    ...
    for await (const raw of body) {
      ...
      const downtimeType = await downtimeTypeRepo.readRawByUnique({ factory_id: raw.factory_id, downtime_type_cd: raw.downtime_type_cd });
      raw.downtime_type_id = unsealArray(downtimeType.raws).downtime_type_id;
    }
    return body;
  }
