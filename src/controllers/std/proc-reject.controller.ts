...
class StdProcRejectCtl extends BaseCtl {
  //#region ✅ Constructor
  constructor() {
    // ✅ 부모 Controller (Base Controller) 의 CRUD Function 과 상속 받는 자식 Controller(this) 의 Repository 를 연결하기 위하여 생성자에서 Repository 생성
    super(StdProcRejectRepo);
  ...
  public convertUniqueToFk = async (body: any[], tenant: string) => {
    ...
    const rejectRepo = new StdRejectRepo(tenant);
    ...
    for await (const raw of body) {
      ...
      const reject = await rejectRepo.readRawByUnique({ factory_id: raw.factory_id, reject_cd: raw.reject_cd });
      raw.reject_id = unsealArray(reject.raws).reject_id;
    }
    return body;
  }
...
export default StdProcRejectCtl;