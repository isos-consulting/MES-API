...
class StdInspItemCtl extends BaseCtl {
  ...
  public convertUniqueToFk = async (body: any[], tenant: string) => {
    ...
    for await (const raw of body) {
      ...
      const inspItemType = await inspItemTypeRepo.readRawByUnique({ factory_id: raw.factory_id, insp_item_type_cd: raw.insp_item_type_cd });
      raw.insp_item_type_id = unsealArray(inspItemType.raws).insp_item_type_id;
      
      const inspTool = await inspToolRepo.readRawByUnique({ factory_id: raw.factory_id, insp_tool_cd: raw.insp_tool_cd });
      raw.insp_tool_id = unsealArray(inspTool.raws).insp_tool_id;
      
      const inspMethod = await inspMethodRepo.readRawByUnique({ factory_id: raw.factory_id, insp_method_cd: raw.insp_method_cd });
      raw.insp_method_id = unsealArray(inspMethod.raws).insp_method_id;
    }
    return body;
  }
