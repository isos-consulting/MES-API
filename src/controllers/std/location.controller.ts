...
class StdLocationCtl extends BaseCtl {
  ...
  repo: StdLocationRepo;

  //#region ✅ Constructor
  constructor() {
    // ✅ 부모 Controller (Base Controller) 의 CRUD Function 과 상속 받는 자식 Controller(this) 의 Repository 를 연결하기 위하여 생성자에서 Repository 생성
    super(new StdLocationRepo());
  ...
  public convertUniqueToFk = async (body: any[]) => {
    ...
    for await (const raw of body) {
      ...
      const store = await storeRepo.readRawByUnique({ factory_id: raw.factory_id, store_cd: raw.store_cd });
      raw.store_id = unsealArray(store.raws).store_id;
    }
    return body;
  }
