...
class StdRejectCtl extends BaseCtl {
  ...
  constructor() {
    ...
    this.fkIdInfos = [
      ...
      {
        key: 'rejectType',
        repo: new StdRejectTypeRepo(),
        idName: 'reject_type_id',
        uuidName: 'reject_type_uuid'
      },
    ];
  };
  //#endregion
  ...
  public convertUniqueToFk = async (body: any[]) => {
    ...
    const rejectTypeRepo = new StdRejectTypeRepo();
    ...
    for await (const raw of body) {
      ...
      const rejectType = await rejectTypeRepo.readRawByUnique({ factory_id: raw.factory_id, reject_type_cd: raw.reject_type_cd });
      raw.reject_type_id = unsealArray(rejectType.raws).reject_type_id;
    }
    return body;
  }
