...
class StdEmpCtl extends BaseCtl {
  //#region ✅ Constructor
  constructor() {
  ...
  public convertUniqueToFk = async (body: any[], tenant: string) => {
    const userRepo = new AutUserRepo(tenant);
    const deptRepo = new StdDeptRepo(tenant);
    const gradeRepo = new StdGradeRepo(tenant);

    for await (const raw of body) {
      const user = await userRepo.readRawByUnique({ id: raw.id });
      raw.uid = unsealArray(user.raws).uid;

      const dept = await deptRepo.readRawByUnique({ dept_cd: raw.dept_cd });
      raw.dept_id = unsealArray(dept.raws).dept_id;

      const grade = await gradeRepo.readRawByUnique({ grade_cd: raw.grade_cd });
      raw.grade_id = unsealArray(grade.raws).grade_id;
    }
    return body;
  }
  ...
  beforeRead = async(req: express.Request) => {
    ...
    const empStatus = req.query.emp_status as string;
    if (![ 'all', 'incumbent', 'retiree' ].includes(empStatus)) { throw new Error('잘못된 emp_status(사원 재직여부) 입력') }
  }
