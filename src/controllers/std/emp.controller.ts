...
import StdEmpService from '../../services/std/emp.service';
...
class StdEmpCtl {
  ...
  public readByWorkings = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      let result: ApiResult<any> = { count: 0, raws: [] };
      const service = new StdEmpService(req.tenant.uuid);
      const params = matchedData(req, { locations: [ 'query' ] });

      result = await service.readByWorkings(params.workings_uuid);
