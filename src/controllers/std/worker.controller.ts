...
class StdWorkerCtl extends BaseCtl {
  ...
  public convertUniqueToFk = async (body: any[]) => {
    ...
    const procRepo = new StdProcRepo();
    const workingsRepo = new StdWorkingsRepo();
    const empRepo = new StdEmpRepo();
    ...
    for await (const raw of body) {
      ...
      const proc = await procRepo.readRawByUnique({ factory_id: raw.factory_id, proc_cd: raw.proc_cd });
      raw.proc_id = unsealArray(proc.raws).proc_id;

      const workings = await workingsRepo.readRawByUnique({ factory_id: raw.factory_id, workings_cd: raw.workings_cd });
      raw.workings_id = unsealArray(workings.raws).workings_id;

      const emp = await empRepo.readRawByUnique({ emp_cd: raw.emp_cd });
      raw.emp_id = unsealArray(emp.raws).emp_id;
    }
