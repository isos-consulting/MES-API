...
class OutReleaseCtl extends BaseCtl {
  ...
  constructor() {
    ...
    super(new OutReleaseRepo());
    this.detailRepo = new OutReleaseDetailRepo();
      ...
      await sequelize.transaction(async(tran) => { 
        for await (const data of req.body) {
          ...
          if (!releaseUuid) {
            ...
            // 📌 Max Seq 계산
            maxSeq = await this.detailRepo.getMaxSeq(releaseId, tran) as number;
          }
          ...
          headerResult = await this.updateTotal(releaseId, releaseUuid, req.user?.uid as number, tran);
  ...
  public readIncludeDetails = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const headerResult = await this.repo.readByUuid(params.release_uuid);
    ...
    try {
      ...
      await sequelize.transaction(async(tran) => { 
        for await (const data of req.body) {
          ...
          const headerResult = await this.updateTotal(data.header[0].release_id, data.header[0].uuid, req.user?.uid as number, tran);
    ...
    try {
      ...
      await sequelize.transaction(async(tran) => { 
        for await (const data of req.body) {
          ...
          const count = await this.detailRepo.getCount(data.header[0].release_id, tran);

          let headerResult: ApiResult<any>;
          if (count == 0) {
            headerResult = await this.repo.delete(data.header, req.user?.uid as number, tran);
          } else {
            headerResult = await this.updateTotal(data.header[0].release_id, data.header[0].uuid, req.user?.uid as number, tran);
          }
