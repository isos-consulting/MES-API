...
class PrdWorkRoutingCtl {
  ...
  // 📒 Fn[readByUuid] (✅ Inheritance): Default ReadByUuid Function
  public readByWorkRoutingOriginUuid = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      let result: ApiResult<any> = { count:0, raws: [] };
      const service = new prdWorkRoutingService(req.tenant.uuid);
      const params = matchedData(req, { locations: [ 'query', 'params' ] });

      result = await service.readByWorkRoutingOriginUuid(params);
