...
class MatReceiveCtl extends BaseCtl {
  ...
  public create = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const detailRepo = new MatReceiveDetailRepo(req.tenant.uuid);
      const incomeRepo = new MatIncomeRepo(req.tenant.uuid);
...
// 📒 Fn[readLotTracking]: 입하기준 lot 추적
  public readLotTracking = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const result = await repo.readLotTrackingToReverse(params);
  ...
  public update = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      await sequelize.transaction(async(tran) => { 
        ...
        const receiveInspResult = await inspResultRepo.readMatReceiveByReceiveUuids(receiveDetailUuids);
  ...
  updateTotal = async (tenant: string, _id: number, _uuid: string, _uid: number, _transaction?: Transaction) => {
    const repo = new MatReceiveRepo(tenant);
    const detailRepo = new MatReceiveDetailRepo(tenant);
