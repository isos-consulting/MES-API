...
import EqmHistoryService from '../../services/eqm/history.service';
...
import StdEquipService from '../../services/std/equip.service';
...
class EqmHistoryCtl {
  ...
  public create = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      const service = new EqmHistoryService(req.tenant.uuid);
  ...
  // 📒 Fn[readHistoryCard]: 설비이력카드 Read Function
  public readHistoryCard = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      let result: ApiResult<any> = { count:0, raws: [] };
      const service = new EqmHistoryService(req.tenant.uuid);
      const equipService = new StdEquipService(req.tenant.uuid);
      const repairHistoryService = new EqmRepairHistoryService(req.tenant.uuid);
      const params = matchedData(req, { locations: [ 'query', 'params' ] });

      const equip = await equipService.readByUuid(params.equip_uuid);
      const histories = await service.read(params);
      const repairHistories = await repairHistoryService.read(params);
      result = {
        raws: [{
          equip: equip.raws[0] ?? {},
          histories: histories.raws,
          repair_histories: repairHistories.raws,
          spare_parts: [],
          images: []
        }],
        count: equip.count + histories.count + repairHistories.count
      }
