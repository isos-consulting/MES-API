...
class MatReturnDetailService {
  tenant: string;
  stateTag: string;
  repo: MatReturnDetailRepo;
  storeRepo: InvStoreRepo;
  fkIdInfos: getFkIdInfo[];

  constructor(tenant: string) {
    this.tenant = tenant;
    this.stateTag = 'matReturnDetail';
    this.repo = new MatReturnDetailRepo(tenant);
  ...
  /**
   * 자재반출상세 데이터의 반출수량 * 단위변환 계수 * 단가 * 환율을 합계금액(total_price)로 입력하여 수정
   * @param datas 자재반출상세 데이터
   * @param uid 입력 사용자ID
   * @param tran DB Transaction
   * @returns total_price가 추가 된 자재반출상세 데이터
   */
  public updateTotalPrice = async (datas: any[], uid: number, tran?: Transaction) => {
    datas = datas.map((data: any) => {
      data.total_price = data.qty * data.convert_value * data.price * data.exchange; 
      return data;
    });
...
export default MatReturnDetailService;