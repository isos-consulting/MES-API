...
const admExcelFormValidation = {
  ...
  read: [
    ...
    query('menu_uuid', '메뉴 UUID').optional({ nullable: true })
			.isUUID().withMessage(value => createValidationError(value, stateTag, errorState.INVALID_READ_PARAM, 400, 'menu_uuid', '메뉴 UUID')),
  ],
  excelFormDownload: [
		query('excel_form_cd', '파일관리 유형UUID').optional({ nullable: true })
  ...
  create: [
    ...
    body('*.reference_menu', '잠조 메뉴').optional({ nullable: true })
      .isString().withMessage(value => createValidationError(value, stateTag, errorState.INVALID_DATA_TYPE, 400, 'reference_menu', '잠조 메뉴')),
